    ...
    '/',
    {
      ...
      schema: {
        ...
        description:
          ...
          'Campos de texto (multipart, todos obrigatórios): `type` (um de `LOST`, `FOUND`, `DONATION`); `title` (string não vazia); `description` (string não vazia); `species` (string livre, ex: cachorro, gato); `latitude` e `longitude` (números); `city` (string não vazia). Além disso, uma ou mais partes de arquivo com a foto do pet (qualquer nome de campo — cada parte do tipo arquivo recebida é tratada como uma foto).\n\n' +
          'Esses campos não aparecem no schema de `body` desta rota porque a validação do Fastify não se aplica a `multipart/form-data` da mesma forma que a um body JSON: eles são lidos manualmente do multipart e validados via `submitPetListingBodySchema.parse(...)` dentro do handler (ver `pets.schema.ts`).',
        tags: ['pets'],
